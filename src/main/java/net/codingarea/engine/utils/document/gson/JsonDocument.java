...
import net.codingarea.engine.utils.document.IDocument;
import net.codingarea.engine.utils.document.IReadable;
...
/**
 * @author CloudNet-v3 | https://github.com/CloudNetService/CloudNet-v3/blob/master/cloudnet-common/src/main/java/de/dytanic/cloudnet/common/document/gson/JsonDocument.java
 * @since 0.0.1
 */
public class JsonDocument implements IDocument<JsonDocument>, Cloneable {
	...
	public JsonDocument clear() {
		for (Entry<String, JsonElement> elementEntry : this.jsonObject.entrySet()) {
	...
	@Override
	public JsonDocument append(@Nonnull String key, @Nonnull Properties properties) {
	...
	public JsonDocument append(Reader reader) {
		return append(GsonParserWrapper.parseReader(reader).getAsJsonObject());
	}
	...
	public JsonDocument read(Reader reader) {
		try (BufferedReader bufferedReader = new BufferedReader(reader)) {
			return this.append(GsonParserWrapper.parseReader(bufferedReader).getAsJsonObject());
		} catch (Exception ex) {
			ex.getStackTrace();
		}
		return this;
	...
	public IReadable read(byte[] bytes) {
		this.append(GsonParserWrapper.parseString(new String(bytes, StandardCharsets.UTF_8)).getAsJsonObject());
		return this;
	}

	public JsonDocument read(String input) {
		try {
			this.append(GsonParserWrapper.parseReader(new BufferedReader(new StringReader(input))).getAsJsonObject());
		} catch (Exception exception) {
			exception.printStackTrace();
		}
		return this;
